...
public class Client implements Watcher, Closeable {
    ...
    void startZK() throws IOException {
        //Connects the process running this class to the zookeeper server
        zk = new ZooKeeper(hostPort, 15000, this);
    }
    
    public void process(WatchedEvent e) { 
        System.out.println(e);
        //Determines whether or not the connection was successful
    ...
    void submitTask(String task, TaskObject taskCtx){
        ...
        /* Creates a new znode in zookeeper in the "tasks" znode. This node already
         * exists because the Master creates it if it didn't exist, and master must
         * run before the client can. This is created because the master has a watcher
         * on any new nodes created within the tasks znode.
         */
    ...
    StringCallback createTaskCallback = new StringCallback() {
        public void processResult(int rc, String path, Object ctx, String name) {
            ...
            case CONNECTIONLOSS:
                ...
                break;
            case OK:
                LOG.info("Client created task name: " + name);
    ...
    void watchStatus(String path, Object ctx){
        ctxMap.put(path, ctx);
        /*
         * This call checks to see if something in the status znode has changed.
         * The master process is the one to add things to the status znode, so
         * if something is there, the callback executes, meaning that a task
         * has been completed and sent back to the client.
         */
        zk.exists(path, 
                statusWatcher, 
                existsCallback, 
    ...
    Watcher statusWatcher = new Watcher(){
        public void process(WatchedEvent e){
            if(e.getType() == EventType.NodeCreated) {
                ...
                /*
                 * Gets the data that was at the path (status znode). This data is
                 * what the master returned ie, the completed task.
                 */
    ...
    DataCallback getDataCallback = new DataCallback(){
        public void processResult(int rc, String path, Object ctx, byte[] data, Stat stat) {
            ...
            case OK:
                ...
                LOG.info("Task " + path + ", received from master:");
                LOG.info(taskResult);
                ...
                /*
                 *  Delete status znode so we don't get errors down the road.
                 */
    ...
    private boolean checkTaskValid(String task){
        RequestValidator validator = new RequestValidator(task);
        boolean requestValid = validator.checkRequestValid();

        if(!requestValid){
            LOG.error("Submitted task: " + task + " was not valid.");
        }
    ...
    public static void main(String args[]) throws Exception { 
        ...
        //Gets the tasks from a file passed through command line argument
        //one at a time.
        String line;
...
//
//
//        c.submitTask("insert keyOne 1", task1);
//        task1.waitUntilDone();
//        c.submitTask("insert keyTwo 2", task5);
//        task5.waitUntilDone();
//        c.submitTask("insert keyThree 3", task2);
//        task2.waitUntilDone();
//        c.submitTask("submit - keyOne keyTwo keyThree", task4);
//        task4.waitUntilDone();
//        c.submitTask("submit * keyTwo keyThree", task3);
//        task3.waitUntilDone();
//
